...
class NowFunctionCCMIT extends EndToEndCCMITBase<byte[]> {
  ...
  @Test
  void should_insert_value_using_now_function_json() throws Exception {
    // given
  ...
  @Test
  void should_insert_value_using_now_function_for_two_dse_columns() throws Exception {
    // given
  ...
  @Test
  void should_insert_value_using_now_function_avro() throws Exception {
    initConnectorAndTask(
        makeConnectorProperties("bigintcol=value.bigint, loaded_at=now(), loaded_at2=now()"));
  ...
  @Test
  void delete_simple_key_json_when_using_now_function_in_mapping() throws Exception {
    // First insert a row...
    session.execute(
