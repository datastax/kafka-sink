  ...
  private final StructToUDTCodec udtCodec1 =
      (StructToUDTCodec)
          new ConvertingCodecFactory(new TextConversionContext())
              .<GenericRecord, UdtValue>createConvertingCodec(
                  udt1, GenericType.of(GenericRecord.class), true);
  ...
  void should_not_convert_from_invalid_external() {
    GenericRecord other =
        new GenericData.Record(
            SchemaBuilder.record("other").fields().optionalInt("a1").endRecord()) {
          {
            put("a1", 32);
          }
        };
    GenericRecord other2 =
        new GenericData.Record(
