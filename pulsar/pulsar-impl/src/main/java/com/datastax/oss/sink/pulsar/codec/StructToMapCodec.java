...
public class StructToMapCodec extends ConvertingCodec<GenericRecord, Map> {
  ...
  StructToMapCodec(ConvertingCodecFactory codecFactory, MapType cqlType) {
  ...
  @Override
  public Map externalToInternal(GenericRecord record) {
    if (record == null) {
      return null;
    }
    ...
    for (Schema.Field field : schema.getFields()) {
      result.put(field.name(), codec.externalToInternal(record.get(field.name())));
    }
  ...
  @Override
  public GenericRecord internalToExternal(Map internal) {
    if (internal == null) {
      return null;
    }
