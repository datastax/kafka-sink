...
start_distributed_worker () {
	...
	echo "----------------------------------------"
	plugin_path=${CONFLUENT_HOME}/share/java,${CONNECTOR_HOME}/kafka-connect-dse-${DSE_CONNECTOR_VERSION}.jar
	sed -i "s#plugin\.path.*#plugin\.path=$plugin_path#" ${KAFKA_EXAMPLES_HOME}/producers/src/main/java/avro/connect-distributed-avro.properties

	${CONFLUENT_HOME}/bin/connect-distributed ${KAFKA_EXAMPLES_HOME}/producers/src/main/java/avro/connect-distributed-avro.properties >> ${CONFLUENT_HOME}/logs/worker-avro-example.log 2>&1 &
...
start_connector () {
	...
	echo "----------------------------------------"
	curl -X POST -H "Content-Type: application/json" -d ${KAFKA_EXAMPLES_HOME}/producers/src/main/java/avro/dse-sink-avro.json "http://localhost:8083/connectors"
}
...
start_producer () {
	...
	echo "----------------------------------------"
	cd ${KAFKA_EXAMPLES_HOME}/producers; mvn clean compile exec:java -Dexec.mainClass=avro.AvroProducer -Dexec.args="$TOPIC_NAME $TOTAL_RECORDS"
}
