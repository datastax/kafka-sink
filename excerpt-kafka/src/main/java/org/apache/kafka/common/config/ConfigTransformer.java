 ...
 * transformations.
 *
 * <p>The default variable pattern is of the form <code>${provider:[path:]key}</code>, where the
 * <code>provider</code> corresponds to a {@link ConfigProvider} instance, as passed to {@link
 * ConfigTransformer#ConfigTransformer(Map)}. The pattern will extract a set of paths (which are
 * optional) and keys and then pass them to {@link ConfigProvider#get(String, Set)} to obtain the
 ...
 * {@link FileConfigProvider} is passed to the {@link ConfigTransformer#ConfigTransformer(Map)}, and
 * a Properties file with contents
 *
 * <pre>
 * fileKey=someValue
 * </pre>
 ...
 * a key "someKey" and a value "${file:/tmp/properties.txt:fileKey}" is passed to the {@link
 * #transform(Map)} method, then the transformed Map will have an entry with key "someKey" and a
 * value "someValue".
 *
 * <p>This class only depends on {@link ConfigProvider#get(String, Set)} and does not depend on
 * subscription support in a {@link ConfigProvider}, such as the {@link
 * ConfigProvider#subscribe(String, Set, ConfigChangeCallback)} and {@link
...
public class ConfigTransformer {
  ...
  /**
   * Creates a ConfigTransformer with the default pattern, of the form <code>${provider:[path:]key}
   * </code>.
   *
  ...
  /**
   * Transforms the given configuration data by using the {@link ConfigProvider} instances to look
   * up values to replace the variables in the pattern.
   *
  ...
  private static String replace(
      Map<String, Map<String, Map<String, String>>> lookupsByProvider,
      String value,
      Pattern pattern) {
    if (value == null) {
      return null;
