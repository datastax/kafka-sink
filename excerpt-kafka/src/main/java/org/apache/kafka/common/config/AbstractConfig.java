 ...
 *
 * <p>This class holds both the original configuration that was provided as well as the parsed
 */
public class AbstractConfig {
  ...
  /**
   * Construct a configuration with a ConfigDef and the configuration properties, which can include
   * properties for zero or more {@link ConfigProvider} that will be used to resolve variables in
   * configuration property values.
   *
   * <p>The originals is a name-value pair configuration properties and optional config provider
   * configs. The value of the configuration can be a variable as defined below or the actual value.
   * This constructor will first instantiate the ConfigProviders using the config provider configs,
   * then it will find all the variables in the values of the originals configurations, attempt to
   * resolve the variables using the named ConfigProviders, and then parse and validate the
   * configurations.
   *
   * <p>ConfigProvider configs can be passed either as configs in the originals map or in the
   ...
   * ConfigProvider properties are not provided, the constructor will skip the variable substitution
   * step and will simply validate and parse the supplied configuration.
   *
   * <p>The "{@code config.providers}" configuration property and all configuration properties that
   * begin with the "{@code config.providers.}" prefix are reserved. The "{@code config.providers}"
   * configuration property specifies the names of the config providers, and properties that begin
   * with the "{@code config.providers..}" prefix correspond to the properties for that named
   * provider. For example, the "{@code config.providers..class}" property specifies the name of the
   * {@link ConfigProvider} implementation class that should be used for the provider.
   *
   * <p>The keys for ConfigProvider configs in both originals and configProviderProps will start
   * with the above mentioned "{@code config.providers.}" prefix.
   *
   * <p>Variables have the form "${providerName:[path:]key}", where "providerName" is the name of a
   * ConfigProvider, "path" is an optional string, and "key" is a required string. This variable is
   * resolved by passing the "key" and optional "path" to a ConfigProvider with the specified name,
   * and the result from the ConfigProvider is then used in place of the variable. Variables that
   * cannot be resolved by the AbstractConfig constructor will be left unchanged in the
   * configuration.
   *
   ...
   * @param configProviderProps the map of properties of config providers which will be instantiated
   *     by the constructor to resolve any variables in {@code originals}; may be null or empty
   ...
   *
   * @param definition the definition of the configurations; may not be null
   * @param originals the configuration properties plus any optional config provider properties; may
   *     not be null
   */
    ...
    for (Map.Entry<String, ?> entry : originals.entrySet()) {
      if (!(entry.getValue() instanceof String))
        throw new ClassCastException(
            "Non-string value found in original settings for key "
                + entry.getKey()
                + ": "
  ...
  /**
   * Gets all original settings with the given prefix, stripping the prefix before adding it to the
   * output.
   *
  ...
  public Map<String, Object> originalsWithPrefix(String prefix, boolean strip) {
    ...
    for (Map.Entry<String, ?> entry : originals.entrySet()) {
      if (entry.getKey().startsWith(prefix) && entry.getKey().length() > prefix.length()) {
        if (strip) result.put(entry.getKey().substring(prefix.length()), entry.getValue());
  ...
  /**
   * Put all keys that do not start with {@code prefix} and their parsed values in the result map
   * and then put all the remaining keys with the prefix stripped and their parsed values in the
   * result map.
   *
   * <p>This is useful if one wants to allow prefixed configs to override default ones.
   *
   * <p>Two forms of prefixes are supported:
   *
   * <ul>
   *   <li>listener.name.{listenerName}.some.prop: If the provided prefix is
   *       `listener.name.{listenerName}.`, the key `some.prop` with the value parsed using the
   *       definition of `some.prop` is returned.
   *   <li>listener.name.{listenerName}.{mechanism}.some.prop: If the provided prefix is
   *       `listener.name.{listenerName}.`, the key `{mechanism}.some.prop` with the value parsed
   *       using the definition of `some.prop` is returned. This is used to provide per-mechanism
   *       configs for a broker listener (e.g sasl.jaas.config)
   * </ul>
   */
  ...
  /**
   * If at least one key with {@code prefix} exists, all prefixed values will be parsed and put into
   * map. If no value with {@code prefix} exists all unprefixed values will be returned.
   *
   * <p>This is useful if one wants to allow prefixed configs to override default ones, but wants to
   ...
   * The configuration may specify either null or an empty string to indicate no configured
  ...
  public <T> List<T> getConfiguredInstances(
      String key, Class<T> t, Map<String, Object> configOverrides) {
   ...
   */
  public <T> List<T> getConfiguredInstances(
      List<String> classNames, Class<T> t, Map<String, Object> configOverrides) {
  ...
  private Map<String, String> extractPotentialVariables(Map<?, ?> configMap) {
    // Variables are tuples of the form "${providerName:[path:]key}". From the configMap we extract
    // the subset of configs with string
   ...
   * from the config providers. returns a map of config key and resolved values.
   *
   ...
   */
  @SuppressWarnings("unchecked")
  private Map<String, ?> resolveConfigVariables(
      Map<String, ?> configProviderProps, Map<String, Object> originals) {
    Map<String, String> providerConfigString;
    Map<String, ?> configProperties;
    Map<String, Object> resolvedOriginals = new HashMap<>();
    // As variable configs are strings, parse the originals and obtain the potential variable
    // configs.
  ...
  private Map<String, Object> configProviderProperties(
      String configProviderPrefix, Map<String, ?> providerConfigProperties) {
  ...
  /**
   * Instantiates and configures the ConfigProviders. The config providers configs are defined as
   * follows: config.providers : A comma-separated list of names for providers.
   * config.providers.{name}.class : The Java class name for a provider.
   * config.providers.{name}.param.{param-name} : A parameter to be passed to the above Java class
   * on initialization. returns a map of config provider name and its instance.
   *
  ...
  private Map<String, ConfigProvider> instantiateConfigProviders(
      Map<String, String> indirectConfigs, Map<String, ?> providerConfigProperties) {
    ...
    for (Map.Entry<String, String> entry : providerMap.entrySet()) {
      try {
        ...
        throw new ConfigException(
            "Invalid config:" + entry.getValue() + " ClassNotFoundException exception occurred", e);
      }
    }
  ...
  /**
   * Marks keys retrieved via `get` as used. This is needed because `Configurable.configure` takes a
   * `Map` instead of an `AbstractConfig` and we can't change that without breaking public API like
   * `Partitioner`.
   */
  ...
  /**
   * ResolvingMap keeps a track of the original map instance and the resolved configs. The originals
   * are tracked in a separate nested map and may be a `RecordingMap`; thus any access to a value
   * for a key needs to be recorded on the originals map. The resolved configs are kept in the
   * inherited map and are therefore mutable, though any mutations are not applied to the originals.
   */
