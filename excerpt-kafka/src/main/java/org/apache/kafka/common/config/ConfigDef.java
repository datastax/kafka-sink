 ...
 * UI.
 *
 * <p>You can provide special validation logic used for single configuration validation by
 * overriding {@link ConfigDef.Validator}.
 *
 * <p>Moreover, you can specify the dependents of a configuration. The valid values and visibility
 * of a configuration may change according to the values of other configurations. You can override
 * {@link ConfigDef.Recommender} to get valid values and set visibility of a configuration given the
 ...
 * </pre>
 *
 * <p>This class can be used standalone or in combination with {@link AbstractConfig} which provides
...
public class ConfigDef {
  ...
  private final Map<String, ConfigDef.ConfigKey> configKeys;
  ...
  public Map<String, Object> defaultValues() {
    ...
    for (ConfigDef.ConfigKey key : configKeys.values()) {
  ...
  /**
   * Define a new configuration with no special validation logic, not dependents and no custom
   * recommender
   *
   * @param name the name of the config parameter
   * @param type the type of the config
  ...
  /**
   * Define a new configuration with no default value, no special validation logic and no custom
   * recommender
   *
   * @param name the name of the config parameter
   * @param type the type of the config
  ...
  /**
   * Define a new configuration with no default value, no special validation logic, no dependents
   * and no custom recommender
   *
   * @param name the name of the config parameter
   * @param type the type of the config
   * @param importance the importance of this config
  ...
  /**
   * Define a new configuration with no group, no order in group, no width, no display name, no
   * dependents and no custom recommender
   *
   * @param name the name of the config parameter
   * @param type the type of the config
   * @param defaultValue the default value to use if this config isn't present
   ...
   */
  public ConfigDef define(
      String name, ConfigDef.Type type, ConfigDef.Importance importance, String documentation) {
  ...
  // not sure if these will be required, so comment them for now
  ...
  Object parseValue(ConfigDef.ConfigKey key, Object value, boolean isSet) {
    Object parsedValue;
    if (isSet) {
      parsedValue = parseType(key.name, value, key.type);
      // props map doesn't contain setting, the key is required because no default value specified -
      // its an error
  ...
  private void parseForValidate(
    ...
    ConfigDef.ConfigKey key = configKeys.get(name);
      ...
      switch (type) {
        ...
        case INT:
          if (value instanceof Integer) {
            ...
            throw new ConfigException(
                name,
                value,
                "Expected value to be a 32-bit integer, but it was a "
        ...
        case LIST:
          if (value instanceof List) return value;
            ...
            else return Arrays.asList(COMMA_WITH_WHITESPACE.split(trimmed, -1));
        ...
        case CLASS:
          if (value instanceof Class) return value;
            ...
            // Use loadClass here instead of Class.forName because the name we use here may be an
            // alias
            // and not match the name of the class that gets loaded. If that happens, Class.forName
            // can
            // throw an exception.
  ...
  public static Map<String, String> convertToStringMapWithPasswordValues(Map<String, ?> configs) {
    ...
    for (Map.Entry<String, ?> entry : configs.entrySet()) {
      ...
      else if (value instanceof List) strValue = convertToString(value, ConfigDef.Type.LIST);
      else if (value instanceof Class) strValue = convertToString(value, ConfigDef.Type.CLASS);
  ...
  /**
   * This is used by the {@link #validate(Map)} to get valid values for a configuration given the
   * current configuration values in order to perform full configuration validation and visibility
   * modification. In case that there are dependencies between configurations, the valid values and
   * visibility for a configuration may change given the values of other configurations.
   */
  public interface Recommender {
     ...
     * @return The list of valid values. To function properly, the returned objects should have the
     *     type defined for the configuration using the recommender.
     */
  ...
  public static class Range implements ConfigDef.Validator {
    ...
    public static ConfigDef.Range atLeast(Number min) {
      return new ConfigDef.Range(min, null);
    }
    ...
    public static ConfigDef.Range between(Number min, Number max) {
      return new ConfigDef.Range(min, max);
    }

    public void ensureValid(String name, Object o) {
      if (o == null) throw new ConfigException(name, null, "Value must be non-null");
      Number n = (Number) o;
  ...
  public static class ValidList implements ConfigDef.Validator {

    final ConfigDef.ValidString validString;

    private ValidList(List<String> validStrings) {
      this.validString = new ConfigDef.ValidString(validStrings);
    }

    public static ConfigDef.ValidList in(String... validStrings) {
      return new ConfigDef.ValidList(Arrays.asList(validStrings));
    }
  ...
  public static class ValidString implements ConfigDef.Validator {
    ...
    public static ConfigDef.ValidString in(String... validStrings) {
      return new ConfigDef.ValidString(Arrays.asList(validStrings));
    }
    ...
    public void ensureValid(String name, Object o) {
      ...
      if (!validStrings.contains(s)) {
        throw new ConfigException(
            name, o, "String must be one of: " + Utils.join(validStrings, ", "));
      }
    }
  ...
  public static class CaseInsensitiveValidString implements ConfigDef.Validator {
    ...
    private CaseInsensitiveValidString(List<String> validStrings) {
      this.validStrings =
          validStrings.stream().map(s -> s.toUpperCase(Locale.ROOT)).collect(Collectors.toSet());
    }

    public static ConfigDef.CaseInsensitiveValidString in(String... validStrings) {
      return new ConfigDef.CaseInsensitiveValidString(Arrays.asList(validStrings));
    }
    ...
    public void ensureValid(String name, Object o) {
      ...
      if (s == null || !validStrings.contains(s.toUpperCase(Locale.ROOT))) {
        throw new ConfigException(
            name, o, "String must be one of (case insensitive): " + Utils.join(validStrings, ", "));
      }
    }
  ...
  public static class NonNullValidator implements ConfigDef.Validator {
    @Override
  ...
  public static class LambdaValidator implements ConfigDef.Validator {
    BiConsumer<String, Object> ensureValid;
    Supplier<String> toStringFunction;

    private LambdaValidator(
        BiConsumer<String, Object> ensureValid, Supplier<String> toStringFunction) {
      this.ensureValid = ensureValid;
      this.toStringFunction = toStringFunction;
    }

    public static ConfigDef.LambdaValidator with(
        BiConsumer<String, Object> ensureValid, Supplier<String> toStringFunction) {
      return new ConfigDef.LambdaValidator(ensureValid, toStringFunction);
    }
  ...
  public static class CompositeValidator implements ConfigDef.Validator {
    private final List<ConfigDef.Validator> validators;

    private CompositeValidator(List<ConfigDef.Validator> validators) {
      this.validators = Collections.unmodifiableList(validators);
    }

    public static ConfigDef.CompositeValidator of(ConfigDef.Validator... validators) {
      return new ConfigDef.CompositeValidator(Arrays.asList(validators));
    }
    ...
    public String toString() {
      ...
      for (ConfigDef.Validator v : validators) {
        if (desc.length() > 0) {
          desc.append(',').append(' ');
        }
  ...
  public static class NonEmptyString implements ConfigDef.Validator {
  ...
  public static class NonEmptyStringWithoutControlChars implements ConfigDef.Validator {

    public static ConfigDef.NonEmptyStringWithoutControlChars nonEmptyStringWithoutControlChars() {
      return new ConfigDef.NonEmptyStringWithoutControlChars();
    }
    ...
    public void ensureValid(String name, Object value) {
      ...
      if (s == null) {
        // This can happen during creation of the config object due to no default value being
        // defined for the
        // name configuration - a missing name parameter is caught when checking for mandatory
        // parameters,
  ...
  public static class ConfigKey {
    ...
    public final ConfigDef.Validator validator;
    public final ConfigDef.Importance importance;
    public final String group;
    public final int orderInGroup;
    public final ConfigDef.Width width;
    public final String displayName;
    public final List<String> dependents;
    public final ConfigDef.Recommender recommender;
    ...
    public ConfigKey(
      ...
      this.type = type;
      this.defaultValue =
          NO_DEFAULT_VALUE.equals(defaultValue)
              ? NO_DEFAULT_VALUE
  ...
  public String toHtmlTable(Map<String, String> dynamicUpdateModes) {
    ...
    b.append("</tr>\n");
    for (ConfigDef.ConfigKey key : configs) {
  ...
  public String toRst() {
    StringBuilder b = new StringBuilder();
    for (ConfigDef.ConfigKey key : sortedConfigs()) {
      if (key.internalConfig) {
        continue;
      }
  ...
  private void getConfigKeyRst(ConfigDef.ConfigKey key, StringBuilder b) {
  ...
  private List<ConfigDef.ConfigKey> sortedConfigs() {
    ...
    List<ConfigDef.ConfigKey> configs = new ArrayList<>(configKeys.values());
  ...
  public void embed(
      ...
      final int startingOrd,
      final ConfigDef child) {
  ...
  /**
   * Returns a new validator instance that delegates to the base validator but unprefixes the config
   * name along the way.
   */
  private static ConfigDef.Validator embeddedValidator(
      final String keyPrefix, final ConfigDef.Validator base) {
  ...
  private static List<String> embeddedDependents(
      final String keyPrefix, final List<String> dependents) {
  ...
  /**
   * Returns a new recommender instance that delegates to the base recommender but unprefixes the
   * input parameters along the way.
   */
  private static ConfigDef.Recommender embeddedRecommender(
      final String keyPrefix, final ConfigDef.Recommender base) {
    if (base == null) return null;
  ...
  /**
   * Converts this config into an HTML list that can be embedded into docs. If <code>
   * dynamicUpdateModes</code> is non-empty, a "Dynamic Update Mode" label will be included in the
   * config details with the value of the update mode. Default mode is "read-only".
   *
   * @param dynamicUpdateModes Config name -&gt; update mode mapping
   */
