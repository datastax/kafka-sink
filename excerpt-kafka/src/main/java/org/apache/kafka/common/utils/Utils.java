...
public final class Utils {
  ...
  private static final String[] BYTE_SCALE_SUFFIXES =
      new String[] {"B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"};

  public static final String NL = System.getProperty("line.separator");

  private static final Logger log =
      LoggerFactory.getLogger(org.apache.kafka.common.utils.Utils.class);
  ...
  /**
   * Read a UTF8 string from the current position till the end of a byte buffer. The position of the
   * byte buffer is not affected by this method.
   *
   * @param buffer The buffer to read from
   * @return The UTF8 string
   */
  ...
  /**
   * Read a UTF8 string from a byte buffer at a given offset. Note that the position of the byte
  ...
  public static byte[] toArray(ByteBuffer buffer, int offset, int size) {
    ...
    if (buffer.hasArray()) {
      System.arraycopy(
          buffer.array(), buffer.position() + buffer.arrayOffset() + offset, dest, 0, size);
    } else {
  ...
  public static <T> T newInstance(String klass, Class<T> base) throws ClassNotFoundException {
    return org.apache.kafka.common.utils.Utils.newInstance(loadClass(klass, base));
  }
   ...
   */
  public static <T> Class<? extends T> loadClass(String klass, Class<T> base)
      throws ClassNotFoundException {
    return Class.forName(
            klass, true, org.apache.kafka.common.utils.Utils.getContextOrKafkaClassLoader())
        .asSubclass(base);
  }
  ...
  public static <T> T newParameterizedInstance(String className, Object... params)
    ...
    try {
      Class<?> c =
          Class.forName(
              className, true, org.apache.kafka.common.utils.Utils.getContextOrKafkaClassLoader());
      ...
      throw new ClassNotFoundException(
          String.format(
              ...
              org.apache.kafka.common.utils.Utils.join(argTypes, ", "), className),
          e);
    } catch (InstantiationException e) {
      throw new ClassNotFoundException(
          String.format("Failed to instantiate " + "%s", className), e);
    } catch (IllegalAccessException e) {
      throw new ClassNotFoundException(
          String.format("Unable to access " + "constructor of %s", className), e);
    } catch (InvocationTargetException e) {
      throw new ClassNotFoundException(
          String.format("Unable to invoke " + "constructor of %s", className), e);
    }
  }
  ...
  public static <K, V> String mkString(
      Map<K, V> map, String begin, String end, String keyValueSeparator, String elementSeparator) {
  ...
  public static Map<String, String> parseMap(
      String mapStr, String keyValueSeparator, String elementSeparator) {
  ...
  /**
   * Converts a Properties object to a Map<String, String>, calling {@link #toString} to ensure all
   ...
   * @param newLength new length for the ByteBuffer. returns ByteBuffer
   */
  ...
  /**
   * Creates a map entry (for use with {@link
   * org.apache.kafka.common.utils.Utils#mkMap(java.util.Map.Entry[])})
   *
   * @param k The key
   * @param v The value
  ...
  /**
   * Get the Context ClassLoader on this thread or, if not present, the ClassLoader that loaded
   * Kafka.
   *
   * <p>This should be used whenever passing a ClassLoader to Class.forName
   */
   ...
   *
   * @throws IOException if any of the close methods throws an IOException. The first IOException is
  ...
  public static void closeQuietly(
      AutoCloseable closeable, String name, AtomicReference<Throwable> firstException) {
    if (closeable != null) {
      try {
        closeable.close();
      } catch (Throwable t) {
   ...
   *     destination buffer
   * @throws IOException If an I/O error occurs, see {@link FileChannel#read(ByteBuffer, long)} for
   *     details on the possible exceptions
   */
  ...
  /**
   * Read data from the input stream to the given byte buffer until there are no bytes remaining in
   * the buffer or the end of the stream has been reached.
   *
  ...
  /**
   * Write the contents of a buffer to an output stream. The bytes are copied from the current
  ...
  public static void writeTo(DataOutput out, ByteBuffer buffer, int length) throws IOException {
    if (buffer.hasArray()) {
      ...
      int pos = buffer.position();
      for (int i = pos; i < length + pos; i++) out.writeByte(buffer.get(i));
    }
  }
  ...
  public static <T> List<T> concatLists(
      List<T> left, List<T> right, Function<List<T>, List<T>> finisher) {
  ...
  public static int to32BitField(final Set<Byte> bytes) {
    int value = 0;
    for (final byte b : bytes) value |= 1 << checkRange(b);
    return value;
  }
