...
import com.datastax.oss.sink.metrics.GlobalSinkMetrics;
import com.datastax.oss.sink.metrics.MetricNamesCreator;
import com.datastax.oss.sink.metrics.MetricsJmxReporter;
...
public class InstanceState {
  ...
  private final Set<RecordProcessor> tasks;
  ...
  void registerTask(RecordProcessor task) {
    tasks.add(task);
  }
   ...
   */
  synchronized boolean unregisterTaskAndCheckIfLast(RecordProcessor task) {
    tasks.remove(task);
    if (tasks.isEmpty()) {
